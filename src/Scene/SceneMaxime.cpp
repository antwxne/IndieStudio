...
SceneMaxime::SceneMaxime(Setting &settings) : AScene(settings), _pressed(false), _isPause(false), _scenePause(settings)
{
    ...
    {
        auto tmp = _objects.emplace_back(std::make_shared<Tank>( _settings._players.at(i).name, coords(_posTank[i].first, 0, _posTank[i].second), coords(10, 10, 10), std::make_pair(Tank::sable, Tank::body), std::make_pair(Tank::darkGreen, Tank::turret)));
        if (i == 0)
