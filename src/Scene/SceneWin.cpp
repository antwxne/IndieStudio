...
namespace win {
    ...
    SceneWin::SceneWin(Setting &settings) : UiScene(settings)
    {
        ...
        // _objects.emplace_back(std::make_shared<TexteUI>(_textPos, std::make_pair(0, 0), "Player " + std::find_if(_settings._players.begin(), _settings._players.end(), [](auto &player){return (player->isWin);})->name, 50, 1, std::make_pair(RGB(), RGB())));
